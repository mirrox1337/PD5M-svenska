			...
			if IsPlayerFreeAiming(player) and IsPedArmed(playerped, 5) then
				...
				elseif flag_hasTarget and GetEntityType(SelectTarget) == 2 then
					...
					if GetEntityType(TargetInVeh) == 1 and not IsPedAPlayer(TargetInVeh) then
						...
						Notify('~r~Ingen Ped sitter i fordonet')
					end
				else
					Notify('~r~Ingen Ped eller Fordon kunde hittas')
				end
			else
	...
	if flag_VehicleSelected == 1 then
		...
		if TargetInVeh ~= 0 and TargetInVeh ~= nil then
			if GetVehicleFuelLevel(targetveh) == 0.0 then
				...
				if flagnotalk then
					Notify('~r~Fordonet kan inte bli stannat')
				else
					if releasecar then
						SetVehicleFuelLevel(targetveh, 1000.0)
						TaskVehicleDriveWander(TargetInVeh, targetveh, 30.0, NormalDrivingBehavior)
						...
						if IsPedInAnyVehicle(playerped, false) then
							Notify('~g~Aktivera~s~ dina ~b~sirener~s~ för att stoppa ~y~bilen~s~.')
...
AddEventHandler('pd5m:int:inittalk', function(target, flaginveh)
	...
	if not flagstopped then
		...
		if flagiscurrentlytalking then
			Notify('~y~Ped~s~ pratar för närvarande med en annan ~b~polis!')
...
AddEventHandler('pd5m:int:stopcar', function(targetveh, TargetInVeh)
	...
	if CutoffInteger > 499 then
		Notify('~r~Bilen buggade ut. Trafikstopp avbröts.')
	...
	if not IsPedInAnyVehicle(playerped, true) then
		...
		if flag_hasTarget and GetEntityType(target) == 1 and GetPedType(target) ~=28 and not IsPedAPlayer(target) then
			...
			if distanceToTarget <= 2.0 then
				...
				if flagallowpersonsearch then
					...
					if #savepeditems == 0 then
						founditems = "Hittade inga föremål."
					else
						for i, item in ipairs(savepeditems) do
							if i == 1 then
								founditems = "Du hittade följade: ~n~" .. item
							else
								if i > 4 then
									secondpost = true
									if i == 5 then
					...
					EndTextCommandThefeedPostMessagetext(PedHeadString, PedHeadString, false, 4, "Resultat", "")
					EndTextCommandThefeedPostTicker(false, false)
			...
			if flag_hasTarget and GetEntityType(targetveh) == 2 then
				...
				if distanceToTarget <= 4.0 then
					if GetVehicleNumberOfPassengers(targetveh) == 0 and IsVehicleSeatFree(targetveh, -1) then
						...
						if flagallowcarsearch then
							...
							BeginTextCommandThefeedPost("TWOSTRINGS")
							AddTextComponentSubstringPlayerName(founditems)
							EndTextCommandThefeedPostMessagetext("CHAR_CALL911", "CHAR_CALL911", false, 4, "Resultat", "")
							EndTextCommandThefeedPostTicker(false, false)
				...
				Notify('~r~Ingen PED eller FORDON hittades.')
			end
		end
	end
end)
		...
		if flag_hasTarget and GetEntityType(target) == 1 and GetPedType(target) ~=28 and not IsPedAPlayer(target) then
			...
			if distanceToTarget <= 2.0 then
				...
				if flagallowbreathalyzer then
					...
					local PedHeadString = GetPedheadshotTxdString(PedHeadHandle)
					BeginTextCommandThefeedPost("TWOSTRINGS")
					AddTextComponentSubstringPlayerName("Andningsapparaten visar en andningsalkoholnivå på ~y~")
					AddTextComponentSubstringPlayerName(savedrunklevel .. " ‰ ~s~or ~y~" .. savedrunklevelBAC .. " BAC~s~.")
					EndTextCommandThefeedPostMessagetext(PedHeadString, PedHeadString, false, 4, "Andningsapparaten", "Resultat")
					EndTextCommandThefeedPostTicker(false, false)
		...
		if flag_hasTarget and GetEntityType(target) == 1 and GetPedType(target) ~=28 and not IsPedAPlayer(target) then
			...
			if distanceToTarget <= 2.0 then
				...
				if flagallowdrugtest then
					...
					if flagdrug then
						BeginTextCommandThefeedPost("TWOSTRINGS")
						AddTextComponentSubstringPlayerName("Drogtestet visar spår av ~y~" .. savedrug .. "~s~ av den misstänktes saliv.")
						AddTextComponentSubstringPlayerName("Ett mer detaljerat test indikerar en koncentration av ~y~" .. savedruglevel .. " ng/ml ~s~ i den misstänktes blod.")
						EndTextCommandThefeedPostMessagetext(PedHeadString, PedHeadString, false, 4, "Drogtest", "Resultat")
						EndTextCommandThefeedPostTicker(false, false)
					else
						BeginTextCommandThefeedPost("TWOSTRINGS")
			...
			if distanceToTarget <= 4.0 then
				if GetVehicleNumberOfPassengers(targetveh) == 0 and IsVehicleSeatFree(targetveh, -1) then
					...
					if flagseizecar then
						...
						BeginTextCommandThefeedPost("TWOSTRINGS")
						AddTextComponentSubstringPlayerName("Du beslag tog bilen och låste den. Ring bärgare för att få bort bilen.")
						EndTextCommandThefeedPostMessagetext("CHAR_CALL911", "CHAR_CALL911", false, 4, "Polisåtgärd", "")
						EndTextCommandThefeedPostTicker(false, false)
					else
						Notify('~r~Du kan inte beslagta denna bil.')
					end
				else
					Notify('~r~Bilen är inte tom.')
				end
		...
		if flag_hasTarget and GetEntityType(target) == 1 and GetPedType(target) ~=28 and not IsPedAPlayer(target) then
			...
			if distanceToTarget <= 2.0 then
				...
				if flagallowitemconfiscation then
					...
					AddTextComponentSubstringPlayerName("Du konfiskerade de misstänkta föremålen.")
					EndTextCommandThefeedPostMessagetext(PedHeadString, PedHeadString, false, 4, "Polisåtgärd", "")
					EndTextCommandThefeedPostTicker(false, false)
				...
				if distanceToTarget <= 4.0 then
					if GetVehicleNumberOfPassengers(targetveh) == 0 and IsVehicleSeatFree(targetveh, -1) then
						...
						if flagallowitemconfiscation then
							...
							BeginTextCommandThefeedPost("TWOSTRINGS")
							AddTextComponentSubstringPlayerName("Du konfiskerade föremålen i bilen.")
							EndTextCommandThefeedPostMessagetext("CHAR_CALL911", "CHAR_CALL911", false, 4, "Polisåtgärd", "")
							EndTextCommandThefeedPostTicker(false, false)
							...
							Notify('~r~Du kan inte konfiskera föremål från den här bilen.')
						end
					else
						Notify('~r~Bilen är inte tom.')
					end
		...
		if flag_hasTarget and GetEntityType(target) == 1 and GetPedType(target) ~=28 and not IsPedAPlayer(target) then
			...
			if distanceToTarget <= 2.0 then
				...
				if flagallowfine then
					FineHeight = "Belopp till böter i SEK."
					FineText = ""
					...
					if (GetOnscreenKeyboardResult()) then
						...
						if result ~= nil then
							if IsInt(result) then
								BeginTextCommandThefeedPost("TWOSTRINGS")
								AddTextComponentSubstringPlayerName("Du bötföll den misstänkte med totalt ~g~" .. result .. "SEK~s~.")
								EndTextCommandThefeedPostMessagetext("CHAR_CALL911", "CHAR_CALL911", false, 4, "Polisåtgärd", "")
								EndTextCommandThefeedPostTicker(false, false)
							else
								Notify('~r~Ange ett giltigt belopp.')
							end
						else
							Notify('~r~Ange ett giltigt belopp.')
						end
			...
			if flag_hasTarget and GetEntityType(targetveh) == 2 then
				...
				if distanceToTarget <= 2.0 then
					...
					if TargetInVeh ~= 0 and TargetInVeh ~= nil and not IsPedAPlayer(TargetInVeh) then
						...
						if flagallowfine then
							FineHeight = "Belopp till böter i SEK."
							FineText = "Ange det belopp du vill böta"
